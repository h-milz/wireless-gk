...
static QueueHandle_t log_evt_queue; // Queue to signal processing task
...
static bool IRAM_ATTR pcnt_intr_handler(pcnt_unit_handle_t unit, const pcnt_watch_event_data_t *edata, void *user_ctx) {
    ...
    //     ++sample_count; 
    loc = 1;
    xQueueSendFromISR(queue, &loc, &high_task_wakeup);
...
static void init_pcnt(void) {
    ...
    pcnt_unit_register_event_callbacks(pcnt_unit, &cbs, log_evt_queue);
...
static void i2s_rx_task(void *args) {
    ...
    while (1) {
        if (i2s_data_ready) {
            ...
            // _log[p].loc = 2;
            // _log[p].time = esp_timer_get_time();
            loc = 2;
            xQueueSendFromISR(log_evt_queue, &loc, NULL);
            // printf ("loop entry %lld\n", esp_timer_get_time());
            // printf ("loop entry %lld\n", _log[p].time);
            // p++;
...
void log_task(void) {
    ...
    while (1) {
        if (xQueueReceive(log_evt_queue, &evt, portMAX_DELAY)) {
            _log[p].loc = evt;
            _log[p].time = esp_timer_get_time();
            p++;
        }
        ...
        if (p > NUM) {
            ...
            for (q=1; q<p; q++) {
                switch (_log[q].loc) {
                    case 1: t = "ISR"; break; 
                    case 2: t = "handler entry"; break; 
                    case 3: t = "after read"; break;
                    case 4: t = "after packing"; break;
                    case 5: t = "loop end"; break; 
                    default: t = ""; break; 
                }
            printf ("%15s %lld diff %lld\n", t, _log[q].time, _log[q].time - _log[q-1].time);
...
void app_main(void) {
    ...
    if (sender) {
        ...
        log_evt_queue = xQueueCreate(100, sizeof(int));
        assert(log_evt_queue != NULL);
